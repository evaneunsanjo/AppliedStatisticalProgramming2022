...
door <- list(choice = numbers)
#assigning a class 
class(door)<-"door"


## Creating a method that takes in numeric value and draw a number, see if they match 
#setting a method that is in fact a function which takes in the door ch

PlayGame <- function(door){
    UseMethod("PlayGame")
  }
  # setting what the function exactly does with that input 
  PlayGame.mechanism <- function(choice){ 
    #draw a random number that represents the door behind which the car is hidden,
    prize <- as.numeric(sample(numbers), 1)
    # compare the two numbers 
    if(pick == prize){
      # print a message congratulating a winnign candidate 
